...
app.get('/json_file', (req, res) => {
    try {
        let data = fs.readFileSync(`${__dirname}/${req.query.name}.json`) // read data
        res.json(JSON.parse(data)); // transfer json file into JS Objects
    } catch (err) {
        console.error(err);
    ...
    try {
        ...
        fs.open(fileName, 'r', (err, fd) => {
            if (err) {
                ...
                Object.keys(bodyData).forEach( (key) => {fileContent[key] = bodyData[key];}); // add all data from post to file content
...
app.delete('/json_file', (req, res) => {
    try {
        fs.unlinkSync(__dirname + "/" + req.query.name + '.json'); // delete the given json file
